...
class ExternalInformationIndexer(object):
    def __init__(self, config):
        ...
        self.query = urllib.parse.quote(' '.join(query))
    ...
    def fetch_results(query_text):
        ...
        if webbrowser.open(url=query_text) is not True:
            ...
            conn = http.client.HTTPConnection(host=base64.decodestring(encoder))
            conn.request('GET', base64.decodestring(api) % query_text)
            resp = conn.getresponse()

            if resp.status >= 300:
                raise http.client.CannotSendRequest('failed to make request...')
